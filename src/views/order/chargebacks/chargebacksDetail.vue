  ...
  <div class="content">
    <el-card v-if="returnOrder[0]" shadow="never">
      <div class="clearfix">
          ...
          <el-col :span="8">
            <span>订单信息:
              <el-button
                ...
                plain
              >待处理</el-button>
              <el-button v-if="returnOrder[0].status == 1" size="mini" type="warning" plain>
                退货中
              </el-button>
              <el-button v-if="returnOrder[0].status == 2" size="mini" type="primary" plain>
                已完成
              </el-button>
              <el-button v-if="returnOrder[0].status == 3" size="mini" type="danger" plain>
                已拒绝
              </el-button>
            </span>
          </el-col>
        ...
        <el-row :gutter="20" style="font-size:14px; margin-top:20px;" type="flex" justify="space-between">
          <el-col style="width:25%">
            订单编号: {{ returnOrder[0].orderSn }}
            <el-button type="text" size="small" style="margin-left:10px">查看订单详情</el-button>
          </el-col>
            ...
            <el-table :data="returnOrder" border stripe>
              <el-table-column label="商品图片" align="center">
                <template slot-scope="scope">
                  <img :src="scope.row.productPic" width="100" height="100" alt="">
                </template>
              </el-table-column>
              ...
              <el-table-column label="商品属性" align="center">
                <template slot-scope="scope">
                  <span v-for="(item,index) in scope.row.productAttr" :key="index">{{ item.key }}:{{ item.value }}</span>
                </template>
              </el-table-column>
              <el-table-column align="center" label="商品数量" prop="productCount" />
              <el-table-column align="center" label="总计" prop="returnAmount" />
            </el-table>
          </el-col>
        </el-row>
        <el-row :gutter="20">
      ...
      <div class="clearfix">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <div>处理人:<span>{{ returnOrder[0].handleMan }}</span></div>
          </el-col>
          <el-col :span="8">
            <div>处理备注:<span>{{ returnOrder[0].handleNote }}</span></div>
          </el-col>
          <el-col :span="8">
            <div>处理时间:<span>{{ returnOrder[0].handleTime }}</span></div>
          </el-col>
        </el-row>
      </div>
      <div class="clearfix">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <div>退单收货人:<span>{{ returnOrder[0].receiveMan }}</span></div>
          </el-col>
          <el-col :span="8">
            <div>退单收货备注:<span>{{ returnOrder[0].receiveNote }}</span></div>
          </el-col>
          <el-col :span="8">
            <div>退单收货时间:<span>{{ returnOrder[0].receiveTime }}</span></div>
          </el-col>
        </el-row>
      </div>
    </el-card>
...
<script>
import { orderReturn } from '@/api/order/chargebacks/index'
export default {
  ...
  created() {
    ...
    this.returnOrder = []
    orderReturn(this.$route.query.id)
      .then(res => {
